#!/bin/bash
##
# PL/SQL Unwrapper
# Usage: $0 inputfile outputfile
IF=${1:-"/dev/stdin"}
OF=${2:-"/dev/stdout"}
exec 3<&0 4>&1 5>&2
exec <$IF >$OF
F8=$(for f in {0..255};do printf '\%03o' $f;done)
T=''
T8=$(echo "$T"|while read -n2 f;do [ -n "$f" ] && printf '\%03o' "0x$f";done)
sed -n -e '/ wrapped[ ]*$/,/^\/$/p'| \
sed -n -e '21,/^[[:space:]]*$/p'| \
base64 -d -i| \
sed '1s/.\{20\}\(.*\)/\1/'| \
tr "$F8" "$T8"| \
perl -e '
use strict;
use warnings;
use Compress::Zlib;
my $x = inflateInit()
  or die "Cannot create a inflation stream\n";
my $input = "";
binmode STDIN;
binmode STDOUT;
my ($output, $status);
while (read(STDIN, $input, 4096))
{
  ($output, $status) = $x->inflate(\$input);
  print $output
    if $status == Z_OK or $status == Z_STREAM_END;
  last if $status != Z_OK;
}
die "inflation failed\n"
  unless $status == Z_STREAM_END;
'| \
sed '$s/\x0$//g'
echo
exec 0<&3 3<&- 1>&4 4>&-
